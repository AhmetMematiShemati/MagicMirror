...
// ##################################################################################
	getDom: function() {
		...
		var dataLimit = ((this.rates.length > 8) ? 3 : 1); 
		...
		for (i in this.rates) {
			...
			if (this.config.showText) {
				...
				currencySpan.innerHTML = this.rates[i].symbol + "/TRY: ";
				rateContainer.appendChild(currencySpan);			
			}

			var rateSpan = document.createElement('span');
			rateSpan.innerHTML = ((this.config.layoutStyle == 'ticker' && i < (this.rates.length - 1)) ? (1/(this.rates[i].rate)).toPrecision(3) + ' &bull; ' : (1/(this.rates[i].rate)).toPrecision(3));
